...
export const makeMessagesRecvSocket = (config: SocketConfig) => {
	...
	const handleGroupNotification = (
		...
		const participantJid: string = (getBinaryNodeChild(child, 'participant')?.attrs?.phone_number || getBinaryNodeChild(child, 'participant')?.attrs?.jid) || participant;
		switch (child?.tag) {
		case 'create':
		...
		case 'not_ephemeral':
			...
			break
		case 'modify':
			const oldNumber = getBinaryNodeChildren(child, 'participant').map(p => p.attrs.phone_number || p.attrs.jid);
	...
	const processNotification = async(node: BinaryNode) => {
		...
		const participant = jidNormalizedUser(node.attrs?.participant_pn || node.attrs?.participant);
		switch (nodeType) {
		case 'privacy_token':
